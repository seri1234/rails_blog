Rails.application.routes.draw do
  ...
  resources :posts,    only: [:show,:create, :destroy]  do                      #/posts/id (post_path(post_id))にgetアクセスでshowアクションを実行。記事詳細画面を表示
                                                                                #/posts(posts_path)にpostアクセスでcreatアクションを実行。記事を投稿する。
                                                                                #/posts/id (post_path(post_id))にdeleteアクセスでdestroyアクションを実行。記事を削除する
    resources :comments, only: [:create]                                        #/posts/posts_id/comentにpostアクセスでcreateアクションを実行。コメントを投稿する
  end
end